...
La arquitectura del servidor \acrshort{oaipmh} (ver figura \ref{fig:oai_architecture}) se basa en un modelo cliente servidor que gira a una base de datos PostgreSQL.

La aplicación MOAI al ser una aplicación WSGI de uso local, requiere de un servidor \acrshort{http}, que redirija el trafico de las peticiones externas a la \acrshort{url} del servidor a la aplicación local.

Por cada petición de recursos que recibe la aplicación, MOAI realiza consultas a la \acrshort{bd} PostgreSQL necesarias para obtener la información de los recursos en cuestión, realizar un procesamiento de las consultas a \acrfull{json}\cite{JSON} convertir los datos resultantes en el documento \acrshort{xml} final que se envía al cliente a través del servidor \acrshort{http} NGINX\cite{NGINX}.
...
El repositorio de \acrshort{labman} consiste en un conjunto de un total de 120 tablas para almacenar toda la información (véase la figura \ref{fig:labmanmodel}). Se realizó dicho grafo para tener una visión general de las relaciones de internas del modelo relacional.

Sabiendo que dicho servidor OAI iba a publicar solo la información de las publicaciones de los grupos de investigación, por lo que se pudo simplificar el modelo relacional completo y a veces incomprensible, debido a el solapamiento de las lineas de que simbolizan relaciones, por uno simplificado y dedicado a las publicaciones (véase \ref{fig:publicationsmodel}), lo cual simplificó la tarea del ``mapeo'', dado que la mayoría de la información podía adquirirse desde dicho grafo a excepción de las relaciones externas a las publicaciones, como pueden ser las entidades \textit{Person}, \textit{Tags} y \textit{Languages}. Para visualizar los atributos de estas últimas se tuvo que seguir haciendo uso del diagrama completo.
...
Para el desarrollo del servidor hizo uso de la documentación que proveía la aplicación MOAI para la modificación de la capa de datos. En esta se especificaban los campos de la especificación de \acrshort{dc} que se podían utilizar para el procesado de información y la generación final del \acrshort{xml}. Estos campos y su definición correspondiente por la propia especificación de \acrshort{dc} son las siguientes:
...
\begin{itemize}
	...
	\item \textbf{description:} Documento que puede incluir, pero no estar limitado a el resumen o la tabla de contenidos del recurso.
	...
	\item \textbf{coverage:} La jurisdicción ajo la que un recurso es relevante.
